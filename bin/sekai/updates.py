...
class updates(commands.Cog):
    ...
    async def addbotupdates(self, ctx, channel: TextChannel = None):
        from utility.apps.sekai.database_formatting import add_channel_to_bot_updates_db
    ...
    async def removebotupdates(self, ctx, channel: TextChannel = None):
        from utility.apps.sekai.database_formatting import rm_channel_from_bot_updates_db
        if ctx.message.author.guild_permissions.administrator:
            if(channel == None):
                channel = ctx.channel
    ...
    async def add_cutoff_tracking(self, ctx, channel: TextChannel = None, interval: int = 60):
        from utility.apps.sekai.database_formatting import add_channel_to_cutoff_db
        if ctx.message.author.guild_permissions.administrator:
            ValidIntervals = [1,60]
