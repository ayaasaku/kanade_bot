...
from utility.apps.sekai.api_functions import (
    get_sekai_area_items_info_api, get_sekai_area_items_level_info_api,
    get_sekai_cards_info_api, get_sekai_characters_info_api,
    get_sekai_music_tags_api, get_sekai_musics_api, get_sekai_user_api)
...
async def get_user_decks(import_id: str, path: str, session: aiohttp.ClientSession):
    api = await get_sekai_user_api(import_id, session)
    path_list = ['leader', 'subLeader', 'member1',
    ...
    for item in area_items:
        ...
        for thing in api2:
            if item_id == thing['id']:
                ...
                embed = defaultEmbed(
                    title=name, description=f'**等級：{item_level}**\n{sentence}')
...
async def get_user_cards(import_id: str, index, path: str, session: aiohttp.ClientSession):
    api = await get_sekai_user_api(import_id, session)
    path_list = ['cardId', 'level', 'masterRank',
                 'specialTraningStatus', 'defaultImage']
    if index == None:
...
async def get_group_music(session: aiohttp.ClientSession):
    ...
    group_list = ['vocaloid', 'light_music_club',
                  'school_refusal', 'idol', 'street', 'theme_park', 'other']
    all_music = {
        'vocaloid': [],
        'light_music_club': [],
        'school_refusal': [],
        ...
        if int(music_id) == music['musicId']:
            ...
            for difficulty in music['userMusicDifficultyStatuses']:
                ...
                if difficulty['userMusicResults'] == empty_list:
                    embed = defaultEmbed(
                        title=f'**{music_name} - {difficulty_name.title()} - 尚未挑戰**', description=f'你尚未挑戰此難度')
                    embed.set_thumbnail(url=cover_url)
                    embed_list.append(embed)
                else:
                    embed = defaultEmbed(
                        title=f'**{music_name} - {difficulty_name.title()}**')
                    embed_list.append(embed)

                    for thing in difficulty['userMusicResults']:
                        if thing['playType'] == 'solo':
                            play_type = '單人'
                        elif thing['playType'] == 'multi':
                            play_type = '多人'
                        ...
                        embed.set_thumbnail(url=cover_url)
                        embed.add_field(
                            name='\u200b', value=f'**{play_type}**', inline=False)
